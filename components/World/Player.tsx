...
export const Player: React.FC = () => {
  ...
  const triggerJump = () => {
    if (isFlying) return; // Cannot jump while flying
  ...
  // --- Skill Logic Handlers (Shared between Keyboard and Virtual Buttons) ---
  const handleFireballAction = () => {
      const now = Date.now();
      // Reduced Cooldown to 5000ms
      if (hasFireball && now - lastFireballTime > 5000) {
          if (groupRef.current) {
              useFireball();
  ...
  useEffect(() => {
      ...
      window.addEventListener('cmd-gem-doubler', handleGemDoublerAction);
      ...
      return () => {
          ...
          window.removeEventListener('cmd-gem-doubler', handleGemDoublerAction);
          window.removeEventListener('cmd-immortal', handleImmortalityAction);
      }
  }, [hasFireball, lastFireballTime, useFireball, hasFlight, isFlying, startFlight, endFlight, lastFlightEndTime, hasGemDoubler, activateGemDoubler, activateImmortality]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isDownKey = e.key === 'ArrowDown' || e.key === 's';
  ...
  }, [status, laneCount, hasDoubleJump, activateImmortality, setManualSlowMotion, hasFireball, lastFireballTime, useFireball, hasFlight, isFlying, startFlight, endFlight, hasGemDoubler, activateGemDoubler, lastFlightEndTime]);
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        ...
        // Avoid gesture detection if touching UI elements (buttons handled separately)
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
            ...
            // Tap for jump on mobile if not swiping? 
            // Or keep existing tap logic? 
            // Currently tap triggers immortality. 
            // Let's keep it, but we have a button now too.
            // Maybe tap center screen = jump is more intuitive for runners?
            // Let's stick to current: Tap = Immortal, Swipe Up = Jump.
            handleImmortalityAction();
        }
    };
  ...
  useFrame((state, delta) => {
    ...
    if (!isJumping.current && !isFlying) {
        ...
        // Flying Pose: Arms forward, Legs straight
    ...
    if (shadowRef.current) {
        ...
        const scale = Math.max(0.2, 1 - (height / 5.0) * 0.8); // Adjusted for flight height
  ...
  useEffect(() => {
     const checkHit = (e: any) => {
        if (isInvincible.current || isImmortalityActive || isFlying) return; // Flying makes you safe from ground collisions
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      <group ref={bodyRef} position={[0, 1.1, 0]}> 
        ...
        {/* Jetpack Flames when Flying */}
        {isFlying && (
            <>
                <mesh position={[-0.1, -0.2, -0.3]} rotation={[Math.PI, 0, 0]}>
